defmodule SpiderWeb.CenterOrderController do
  ...
  def index(conn, _params) do
    center_orders = CenterOrders.list_center_orders() |> Repo.preload(:livestock_sales)
    render(conn, "index.json", center_orders: center_orders)
  end
  ...
  def create(conn, %{"center_order" => center_order_params}) do
    with {:ok, %CenterOrder{} = center_order} <- CenterOrders.create_center_order(center_order_params) do
      ...
      |> render("show.json", center_order: center_order |> Repo.preload(:livestock_sales))
    end
  end

  def show(conn, %{"id" => id}) do
    center_order = CenterOrders.get_center_order!(id)
    render(conn, "show.json", center_order: center_order |> Repo.preload(:livestock_sales))
  end
