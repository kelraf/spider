defmodule SpiderWeb.DlivestockImageController do
  ...
  def create(conn, %{
        "d_livestock_image" => dlivestock_image_params,
    ...
    with {:ok, %DlivestockImage{} = dlivestock_image} <-
           DlivestockImages.create_dlivestock_image(data) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"id" => id, "dlivestock_image" => dlivestock_image_params}) do
    ...
    with {:ok, %DlivestockImage{} = dlivestock_image} <-
           DlivestockImages.update_dlivestock_image(dlivestock_image, dlivestock_image_params) do
      render(conn, "show.json", dlivestock_image: dlivestock_image)
    end
  end
